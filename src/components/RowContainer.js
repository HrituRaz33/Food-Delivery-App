...
const RowContainer = ({ flag, data, scrollValue }) => {
    ...
    return (
        <div
            ref={RowContainer}
            className={`w-ful my-12 flex items-center gap-3 ${flag ? "overflow-x-scroll scrollbar-none" : "overflow-x-hidden"}`}>
            {
                data && data.map(item => (
                    <div key={item.id} className='w-full min-w-[300px]  md:w-[300px] h-auto rounded-lg p-2 bg-white shadow-md backdrop-blur-lg  my-12 hover:drop-shadow-xl'>
